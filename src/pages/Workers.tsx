...
export default function Workers() {
  ...
  const getWorkerActiveSuits = (worker: Worker) => {
    return data.orders.flatMap(o => {
      ...
      return o.suits
        .filter(s => s.workers && Object.values(s.workers).includes(worker.id))
        .map(s => ({
          orderId: o.id,
          suitId: s.id,
    ...
    <div className="space-y-4 pb-4">
      ...
      <div className="space-y-3">
        {filtered.map(worker => {
          ...
          const totalCompleted = data.orders.flatMap(o => o.suits).filter(s => s.workers && Object.values(s.workers).includes(worker.id) && s.status === 'delivered').length;
      ...
      {showHisaab && (() => {
        ...
        const totalEarnedAll = data.orders.reduce((sum, o) => {
          return sum + o.suits.filter(s => s.workers && Object.values(s.workers).includes(showHisaab.id)).reduce((ss, s) => {
